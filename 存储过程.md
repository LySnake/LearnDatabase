...
-- 用户变量定义使用SET关键字直接定义，变量名要以@开关
-- 用户变量初始值的设置方式是直接使用=号初始化
...
begin
	if a = 1 then 
		...
		inset into students(stu_num, stu_name, stu_gender, stu_age, cid, remark) values('20210110', '小花', '女', 18, 1, '...');
	end if;
end;
```
...
```sql
-- 删除存储过程
-- drop 删除数据库中的对象，如数据库、数据表、列、存储过程、视图、触发器、索引...
-- delete 删除数据表中的数据
...
5. 完整示例
   ...
   begin
   	...
   	while i < num do 
   		...
   		FETCH myCursor INFO bname, bauthor, bprice;
